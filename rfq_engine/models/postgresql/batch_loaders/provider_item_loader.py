# -*- coding: utf-8 -*-
"""PostgreSQL batch loader for ProviderItem entity.

Batch loads ProviderItemModel records keyed by (partition_key, provider_item_uuid).
"""
    ...
    def batch_load_fn(self, keys: List[Tuple[str, str]]) -> Promise:
        ...
        if unique_keys:
            try:
                ...
                for pk, piu in unique_keys:
                    if pk not in pk_groups:
                        pk_groups[pk] = []
                    pk_groups[pk].append(str(piu) if piu else "")
                ...
                for pk, uuids in pk_groups.items():
                    rows = (
                        ...
                        .filter(
                            ...
                            ProviderItemModel.provider_item_uuid.in_(
                    ...
                    for row in rows:
                        key = (row.partition_key, str(row.provider_item_uuid))
